...
def main():
    ...
    for s in seqs_combined:
        ...
        results.append({"hour": f"{h:02d}:00-{h:02d}:59", "pattern": pattern, "pnl": pnl, "out": out})

    df = pd.DataFrame(results)
    
    print("\n=======================================================")
    print("ANALISI ORARIA COMPLETA (LIVELLO 3 - H24)")
    print("=======================================================\n")
    
    hourly = df.groupby(['hour', 'pattern']).apply(
        lambda x: pd.Series({
            'Trades': len(x),
            'Net_PnL': x['pnl'].sum(),
            'Win_Rate': (x['out'] == 'win').mean() * 100,
            'Gross_Win': x.loc[x['pnl'] > 0, 'pnl'].sum(),
            'Gross_Loss': abs(x.loc[x['pnl'] < 0, 'pnl'].sum())
        })
    )
    
    hourly['Profit_Factor'] = hourly.apply(lambda row: row['Gross_Win'] / row['Gross_Loss'] if row['Gross_Loss'] > 0 else float('inf'), axis=1)
    
    hourly = hourly.reset_index()
    hourly = hourly.sort_values(['hour', 'pattern'])
    
    print(hourly[['hour', 'pattern', 'Trades', 'Net_PnL', 'Profit_Factor', 'Win_Rate']].round(2).to_string(index=False))
