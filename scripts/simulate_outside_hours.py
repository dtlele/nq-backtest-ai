...
def main():
    ...
    for s in seqs_combined:
        ...
        if not rule: 
            ...
            t_val = h * 60 + m
            if dt_obj.strftime("%A") not in rule["days"]: is_in_v2 = False
            elif not (9*60+30 <= t_val < 16*60): is_in_v2 = False
            elif get_session_label(t_val) not in rule["sessions"]: is_in_v2 = False
            elif rule.get("exclude_10am", False) and (9*60+55 <= t_val <= 10*60+5): is_in_v2 = False
        ...
        results_out.append({"pattern": pattern, "pnl": pnl, "out": out})
    ...
    print_stats(results_out, "ORARI FUORI DAI FILTRI (Notte, Mattina Presto, ecc.) CON NUOVI FILTRI (10m+30m)")
