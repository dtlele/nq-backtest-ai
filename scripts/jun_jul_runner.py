"""June-July 2025 weekly parallel runner - 8 processes (4 weeks each month).
Setup: prod2-yellow with ONLY time gate change (no slippage, no trailing re-tune).
"""
import os
import sys
import subprocess
...
def week_ranges(start_date, end_date):
    ...
    while d <= end:
        ...
        if fri > end:
            fri = end
        week_label = f"jun_jul_w{week_num:02d}_{d.strftime('%Y%m%d')}_{fri.strftime('%Y%m%d')}"
        yield (week_label, d.strftime("%Y%m%d"), fri.strftime("%Y%m%d"))
        d = fri + timedelta(days=3)
        week_num += 1

WEEKS = list(week_ranges("20250602", "20250731"))
print(f"Generated {len(WEEKS)} weeks for June-July 2025:")
for n, s, e in WEEKS:
    print(f"  {n}: {s} -> {e}")
...
def launch_week(name, start_date, end_date):
    run_dir = ROOT / "agent_memory_jun_jul" / name
    run_dir.mkdir(parents=True, exist_ok=True)
    log_file = ROOT / "output" / f"jun_jul_{name}.log"
    env = os.environ.copy()
    ...
    # prod2-yellow COMPLETE: slippage + trailing re-tune + time gate 10:00
    env['TRAIL_TRIGGER_RR'] = '0.8'  # re-tuned trailing trigger
    env['TRAIL_LOCK_50_RR'] = '1.5'  # 50% lock at 1.5R
    env['TRAIL_LOCK_75_RR'] = '2.5'  # 75% lock at 2.5R
    env['BACKTEST_SLIPPAGE_PTS'] = '0.5'  # 0.5pt slippage per side
...
def main():
    print('=' * 80)
    print(f'JUNE-JULY 2025 WEEKLY RUNNER - {len(WEEKS)} processes (time gate 10:30 only)')
    print('=' * 80)
    processes = []
    # Launch in 2 waves of 4
