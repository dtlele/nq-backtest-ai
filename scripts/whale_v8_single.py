"""
WHALE v8 - SINGOLA CONFIGURAZIONE (da lanciare in 4 terminali paralleli)
Usage: python whale_v8_single.py A|B|C|D
"""
import os, sys, glob
import pandas as pd
...
if len(sys.argv) < 2 or sys.argv[1] not in ('A','B','C','D'):
    print("Usage: python whale_v8_single.py A|B|C|D")
    sys.exit(1)
...
CONFIGS = {
    'A': {'name': 'A: v7 base (70-190, wick>=1)',
          'min_size': 70, 'max_size': 190, 'max_bar_range': 150.0, 'min_wick': 1.0,
          'sessions': [(time(9,45), time(12,0)), (time(13,30), time(15,15))]},
    'B': {'name': 'B: size 50-200, wick>=0.5',
          'min_size': 50, 'max_size': 200, 'max_bar_range': 150.0, 'min_wick': 0.5,
          'sessions': [(time(9,45), time(12,0)), (time(13,30), time(15,15))]},
    'C': {'name': 'C: size 40-250, wick>=0.5, range<=200',
          'min_size': 40, 'max_size': 250, 'max_bar_range': 200.0, 'min_wick': 0.5,
          'sessions': [(time(9,45), time(12,0)), (time(13,30), time(15,15))]},
    'D': {'name': 'D: size 50-200, no wick, +open/close session',
          'min_size': 50, 'max_size': 200, 'max_bar_range': 150.0, 'min_wick': 0.0,
          'sessions': [(time(9,30), time(12,0)), (time(13,30), time(15,30))]},
}

cfg = CONFIGS[WHICH]
OUT_CSV = os.path.join(OUT_DIR, f"whale_v8_config_{WHICH}.csv")
...
def process_file(filepath, cfg):
    ...
    if pd.isna(median_p) or median_p <= 0: return []
    ...
    for ts, te in cfg['sessions']:
        gold_mask |= (t_all >= ts) & (t_all < te)
    df_gold = raw[gold_mask & raw['side'].isin(['A','B'])].copy()
    if df_gold.empty: return []
    ...
    if cfg['min_wick'] > 0:
        ...
        on_wick = pd.Series(True, index=bars.index)

    signal_bars = bars[size_ok & on_wick].copy()
    if signal_bars.empty: return []
    ...
    for signal_ts, sig_row in signal_bars.iterrows():
        entry_min_start = signal_ts + pd.Timedelta(minutes=1)
        entry_min_end   = entry_min_start + pd.Timedelta(minutes=1)
        entry_cands = df_exit[(df_exit['ts_eastern'] >= entry_min_start) &
                               (df_exit['ts_eastern'] <  entry_min_end)]
        if entry_cands.empty: continue
        ...
        pnl_pts = None; exit_ts = None; exit_price = None; exit_reason = 'TIME_EXIT'
        ...
        if pnl_pts is None:
            ...
            exit_price = last['price'].iloc[0]; exit_ts = last['ts_eastern'].iloc[0]
            pnl_pts = exit_price - entry_price
        trades.append({'entry_ts': entry_ts, 'exit_ts': exit_ts, 'exit_reason': exit_reason,
                        'pnl_pts': round(pnl_pts,4), 'net_pnl': round(pnl_pts*pv - cost, 2)})
        if ONE_TRADE_ONLY: busy_until_ts = exit_ts
    return trades


files = sorted(glob.glob(os.path.join(DATA_DIR, "glbx-mdp3-*.trades.csv")))
print(f"[{WHICH}] {cfg['name']}")
print(f"[{WHICH}] {len(files)} file in corso...")

all_trades = []
for i, f in enumerate(files):
    all_trades.extend(process_file(f, cfg))
    if (i+1) % 50 == 0:
        print(f"[{WHICH}] {i+1}/{len(files)} file | trade finora: {len(all_trades)}")

tdf = pd.DataFrame(all_trades)
tdf.to_csv(OUT_CSV, index=False)

if tdf.empty:
    print(f"[{WHICH}] 0 trade")
else:
    total = len(tdf); wins = (tdf['net_pnl'] > 0).sum()
    wr = wins/total*100
    gw = tdf[tdf['net_pnl']>0]['net_pnl'].sum()
    gl = abs(tdf[tdf['net_pnl']<0]['net_pnl'].sum())
    pf = gw/gl if gl>0 else float('inf')
    tdf['eq'] = tdf['net_pnl'].cumsum(); tdf['pk'] = tdf['eq'].cummax()
    maxdd = (tdf['pk'] - tdf['eq']).max()
    print(f"\n{'='*55}")
    print(f"[{WHICH}] RISULTATI: {cfg['name']}")
    print(f"  Trade: {total} ({total/441:.1f}/gg) | WR: {wr:.1f}% | PF: {pf:.2f}")
    print(f"  Net PnL: ${tdf['net_pnl'].sum():,.0f} | MaxDD: ${maxdd:.0f}")
    print(f"  TP:{(tdf['exit_reason']=='TP').sum()} SL:{(tdf['exit_reason']=='SL').sum()} TIME:{(tdf['exit_reason']=='TIME_EXIT').sum()}")
    print(f"  CSV: {OUT_CSV}")
