        ...
        if fabio_signal.direction in ['long', 'short'] and fabio_signal.confidence >= 55:
            ...
            if os.environ.get('FABIO_MODE', 'scalper').lower() == 'scalper':
                ...
                _audit_sys = (
                    ...
                    "A junior scalper proposes the trade below. Your job is to actively TRY to kill it —\n"
                    "vote 'confirm' ONLY if you failed to find a real invalidation. Be brutally honest:\n"
                    "don't reject setups just because they look 'imperfect' if the structure is sound.\n"
                    "\n"
                    "Check in order:\n"
                    "1. BIAS: trade direction against drive regime? (drive_up veto shorts, drive_down veto longs)\n"
                    "2. FLOW: does delta/CVD actually confirm, or is it being rationalized?\n"
                    "3. LOCATION: is there a STRUCTURAL ANCHOR (POC, VAH/VAL, IB edge, OR a Big Trade wall)?\n"
                    "   - IMPORTANT: the candidate.wall may have n_trades=0, but check 'BIG TRADES' section:\n"
                    "     if a Big Trade with size>=150 sits on the correct side of entry, it IS the wall.\n"
                    "   - Do NOT reject a trade solely because candidate.wall_trade_count=0.\n"
                    "4. TIMING: opening rotation (9:30-9:45 ET), lunch chop (11:45-13:15), late session (15:15+)?\n"
                    "   - The TIME block is in UTC; the gate triggers on these ET windows.\n"
                    "   - Lunch is OK if bias is strong drive_aligned.\n"
                    "5. FAILED AUCTION risk: is price beyond IB with weak acceptance / collapsing delta?\n"
                    "\n"
                    "NOTES:\n"
                    "- GEX regime='unknown' = no penalty (we don't have real GEX data, don't penalize).\n"
                    "- A trade with valid Big Trade wall + flow confirmation + bias aligned = CONFIRM.\n"
                    "- Reject only on FACTUAL invalidation, not aesthetic concerns.\n"
                    "\n"
