            ...
            if os.environ.get('FABIO_MODE', 'scalper').lower() == 'scalper':
                ...
                if _audit_ver_str == 'v2':
                    ...
                    _audit_sys = (
                        ...
                        "\n"
                        "R3 ABSORPTION EXCEPTION (CRITICAL — read carefully):\n"
                        "  Big SELLs in front of a LONG do NOT contradict if the bid is absorbing:\n"
                        "    - bar.delta > 0 (the current bar closed with positive delta) AND\n"
                        "    - cv_delta_30m >= 0 (cumulative delta is not bearish) AND\n"
                        "    - The big SELLS are AT the wall (candidate.wall_level) being defended,\n"
                        "      not below it (selling INTO a level buyers defend = absorption = bullish).\n"
                        "  In this case: SKIP R3, do NOT reject. The big sells are EVIDENCE of\n"
                        "  institutional demand, not opposition.\n"
                        "  Mirror rule for SHORT: bar.delta < 0 + cv_delta_30m <= 0 + big BUYs above\n"
                        "  price (defended from above) = absorption = bearish → SKIP R3.\n"
                        "\n"
                        "  WRONG APPLICATION EXAMPLES (DO NOT DO THIS):\n"
                        "  ✗ 'big SELLs contradict long' when delta=+500, cv_delta=+1500 (those sells\n"
                        "    were absorbed, this is bullish evidence)\n"
                        "  ✗ 'big BUYs contradict short' when delta=-500, cv_delta=-1500 (those buys\n"
                        "    were absorbed by sellers, this is bearish evidence)\n"
                        "\n"
                        "  CORRECT APPLICATION:\n"
                        "  ✓ 'big SELLs contradict long' ONLY when delta<=0 (sellers won the bar)\n"
                        "  ✓ 'big BUYs contradict short' ONLY when delta>=0 (buyers won the bar)\n"
                        "\n"
                ...
                try:
                    _raw = llm_ask(_audit_sys, _audit_msg, model=os.environ.get('AUDIT_MODEL', os.environ.get('OPENROUTER_MODEL', 'minimax/minimax-m2')),
