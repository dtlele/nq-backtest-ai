...
13. MACROECONOMIC NEWS CALENDAR SAFETY: If there is an upcoming news event within <= 5 minutes in the economic calendar, VETO the trade immediately (confirmation=false, confidence=0) unless there is a massive institutional volume anomaly (NAV spike).
14. TRAPPED FOLLOW-THROUGH VALIDATION: Reject/Veto any setup if the M1 footprint analysis shows a FAILED / RELEASED trap or if a trap lacks a CONFIRMED follow-through (e.g. price and delta did not move in the trade's direction).
15. LEVEL MATRIX & AMT PROFILE: Cross-reference the "Session Level Matrix" and the "AMT Session Profile" (Normal, Normal Variation, Neutral, etc.). If the Day Profile is Neutral or Non-Trend, do not confirm breakout trades; only confirm reversals off the absolute extremes.
16. ANTI-DOJI & EXHAUSTION RECOVERY RULE: Veto any trade if the entry is proposed directly after a Doji or narrow-body indecisive candle, unless it is a clear exhaustion-recovery setup. An exhaustion-recovery setup requires that the pullback candle shows a massive reduction in delta and volume (exhaustion), followed by a clear engulfing or wide-body close in the direction of the trade (the recovery) indicating institutional participation on low/rising liquidity. Do not enter on the Doji itself; wait for the first confirmed body close in the trend direction.
17. INSTITUTIONAL WALL CLEARANCE: Do not confirm a LONG trade if there is an active Seller Wall (Resistance/Absorption) at the recent highs and the candidate entry price is below it, unless we have already had a clear engulfing candle body close above that wall level (accepting higher prices). The same applies to SHORT trades near active Buyer Walls.
