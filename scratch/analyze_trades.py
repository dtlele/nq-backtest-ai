...
for key in sorted(trades.keys()):
    ...
    date = t.get('date', '')
    if date not in ['2025-11-13', '2025-11-14', '2025-11-17']: continue # Just show recent days to keep it clean
    ...
    # Check if the initial stop is what we want. The 'stop' field in JSON is the EXIT price or trailing stop!
    # We don't have the initial stop recorded in the JSON for trailing stops.
    # We do have it for losses. Let's calculate the risk for losses.
    if pnl_usd < 0:
