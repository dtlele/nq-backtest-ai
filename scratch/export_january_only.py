...
# Find all January 2025 JSON files
dates_to_process = []
for f in sorted(list(OUTPUT_DIR.glob("2025-01-*.json"))):
    match = re.match(r'(\d{4})-(\d{2})-(\d{2})', f.stem)
    if match:
        dates_to_process.append(f.stem.replace("-", ""))

print(f"Processing January dates: {dates_to_process}")
...
print("All January dates processed successfully!")
