...
def run_correlation_mining():
    ...
    # ── MINING CORRELATIONS ──
    # We will build a list of samples with multiple features:
    # - hour (ET)
    # - hour_bucket (first 2 hours: 09:30-11:30 vs rest)
    # - day_of_week
    # - ib_loc (inside_ib, above_ibh, below_ibl)
    # - va_loc (above_vah, inside_va, below_val)
    # - poc_loc (above_poc, below_poc)
    # - day_type
    # - vwap_loc (above, below, unknown)
    # - stop_dist_bucket
    # - pnl
    # - win (1 or 0)
    ...
    # ── ANALYSIS 2: MULTI-FEATURE COMBINATORIAL SEARCH ──
    # We want to search for 2-feature and 3-feature combinations that yield high performance
    print("\n" + "="*80)
    print("COMBINATORIAL SEARCH: GOLDEN COMBINATIONS (Win Rate >= 45% & N >= 8)")
    print("="*80)
    
    # Let's search 2-feature combinations
    ...
    print("\n--- Top 15 Triple-Feature Combinations ---")
    for key, count, wins, wr, pnl in sorted(valid_combos_3, key=lambda x: x[4], reverse=True)[:15]:
