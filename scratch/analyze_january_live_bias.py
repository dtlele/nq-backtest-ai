...
def analyze_january_live():
    ...
    # Load reasoning log for January 2025
    try:
        with open(log_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    if not date_str.startswith('2025-01'):
                        continue
        ...
        print(f"Error reading reasoning log: {e}")
        return

    # Load trades log for January 2025
    ...
    print(f"=== JANUARY 2025 LIVE CANDIDATES & BIAS ANALYSIS ===")
    ...
    for date in sorted(daily_stats.keys()):
        ...
        # Details of trades if taken
        if stats['trades']:
            for i, t in enumerate(stats['trades']):
                print(f"     -> T{i+1}: {t.get('setup_type')} LONG entry={t.get('entry')} exit={t.get('exit_price')} reason={t.get('exit_reason')} pnl={t.get('pnl_usd')}")

    print("-" * 105)
    print(f"Total Cumulative P&L for January 2025 so far: ${total_pnl:,.2f}")

if __name__ == '__main__':
    analyze_january_live()
